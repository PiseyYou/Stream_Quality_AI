...
#include "encode_close_process.h"
//*********************
#include <locale.h>
#include <wchar.h> 
#include <fstream>
...
void *thread_close(void *threadarg);
...
int main(int argc, char** argv) {
    ...
    cout << "test the debug issue is OK " << endl;
	...
	// There are argc-1 URLs: argv[1] through argv[argc-1].  Open and start streaming each one:		
		...
		{
//open rtsp url and create a thread named thread_close to stop decoder and encoder after 10 seconds
			#ifdef RTSP_DECODER
			...
			for(int i=0;i<line_count;i++)
			{
				...
				{
					cout << "create decoder success." << endl;
				}
			}
			#endif
			#ifdef RTSP_CLOSE
			struct thread_data td1[DETECTORNUM];
			for(int i=0;i<line_count;i++)
			{
				td1[i].thread_id = i;
				int rc = pthread_create(&rtsp_mpp[i],NULL,thread_close,(void *)&td1[i]);
				usleep(1000);
				if (rc)
				{
				    cout << "Error:无法创建线程," << rc << endl;
				...
				{
					cout << "create thread_close success." << endl;
				}
			}
			#endif
			sleep(3);   // sleep 3 seconds to make sure decoder and encoder were init completely
			for(int i=0;i<line_count;i++)
			{	
...
// if only create decoder,this thread will normall exit;if also create encoder,this thread will wait for return value
void *thread_decoder(void *threadarg)
{       
	struct thread_data *my_data;
   	my_data = (struct thread_data *) threadarg;
...
void DummySink::afterGettingFrame(unsigned frameSize, unsigned numTruncatedBytes,
  ...
  for(int i=0;i<line_count;i++)
  {
    ...
    {
      //cout <<"i=" << i << ";frameSize = " << frameSize << endl;
      int flag = ndecoder[i].decode_h264(fReceiveBuffer,frameSize,flag_close);
      if(flag == 1)
      {
