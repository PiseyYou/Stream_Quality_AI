...
bool CRTMPStream::ReadOneNaluFromBuf(NaluUnit &nalu,unsigned char *pH264Buffer)
{
	int i = 0;
	while(i<1000)
	{
		if(pH264Buffer[i++] == 0x00 &&
			pH264Buffer[i++] == 0x00 &&
			pH264Buffer[i++] == 0x00 &&
			pH264Buffer[i++] == 0x01
			)
		{
			nalu.type = pH264Buffer[i]&0x1f;
			nalu.data = &pH264Buffer[i];
...
bool CRTMPStream::ReadOneNaluFromBuf(NaluUnit &nalu,unsigned char *pH264Buffer,int len)
{
	int i = m_nH264Pos;
	while(i<len-4)
	{
			...
			)
		{
			...
			while (pos<len-4)
			{
				if(pH264Buffer[pos++] == 0x00 &&
					pH264Buffer[pos++] == 0x00 &&
					pH264Buffer[pos++] == 0x00 &&
					pH264Buffer[pos++] == 0x01
					)
				{
					break;
				}
