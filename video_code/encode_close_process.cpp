...
int NVEncoder::Encode_proc(string rtmp_url,int yuv_width,int yuv_height)
{
	...
	while(1) 
	{
		...
		cout << "delete enc..." << endl;
		delete ctx.enc;  //we should delete enc,but there were some wrong in capture_plane.
		break;
	}
...
//we create output_plane and capture_plane,and can release output_plane,but failed to release capture_plane
void NVEncoder::EnRelease(bool flag)
{
	//is_stop_thread = flag;
	if(flag)
	{
		...
		cout << "NVEncoder::EnRelease getNumBuffers=" << ctx.enc->capture_plane.getNumBuffers() << endl;
		/*for (uint32_t i = 0; i < ctx.enc->capture_plane.getNumBuffers(); i++)
		{
			struct v4l2_buffer v4l2_buf;
			struct v4l2_plane planes[MAX_PLANES];
			...
			cout << "dqBuffer i=" << i << endl;
			ret = ctx.enc->capture_plane.dqBuffer(v4l2_buf, &enc_buffer, NULL,3);//i
			if (ret < 0)
			{
			    cerr << "EnRelease:Error while queueing buffer at capture plane" << endl;
			    //abort(&ctx);
			}
		}*/
		bool eos = false;
		...
		/*ret = ctx.enc->capture_plane.stopDQThread();  //stop capture_plane thread
		if(ret == 0)
		{
			cout << "stop DQThread success." << endl;
		}*/
